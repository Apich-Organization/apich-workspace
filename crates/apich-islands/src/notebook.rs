//! "Integrate python and r into the tables: just like a python notebook and r notebook" (direct user request).
//!
//! Real Rust island for the per-table notebook (see
...
/// Output image generated by notebook execution.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NotebookCellImageData {
    /// Image asset name or identifier.
    pub name: String,
    /// Base64 data URI of the image (e.g. `data:image/png;base64,...`).
    pub data_uri: String,
}

/// Serialized cell data model for interactive notebooks.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NotebookCellData {
    /// Cell identifier.
    pub id: i64,
    /// Execution kernel language (e.g., "python", "r", "rust", "typst").
    pub language: String,
    /// Source code content.
    pub code: String,
    ...
    let cell_views = move || {
        ...
        all_cells
            ...
            .map(|(idx, cell)| {
                ...
                let is_first = idx == 0;
                let is_last = idx.saturating_add(1) == n_cells;
                view! {
                    <div style="border:1px solid var(--border-subtle); border-radius:8px; margin-bottom:0.85rem; overflow:hidden;">
                        <div style="display:flex; justify-content:space-between; align-items:center; padding:0.5rem 0.75rem; background:var(--bg-muted); border-bottom:1px solid var(--border-subtle);">
                            ...
                            <div style="display:flex; gap:0.4rem; align-items:center;">
                                ...
                                <form method="post" action=format!("/projects/{}/table/notebook/cell-move", project_id_move)>
                                    <input type="hidden" name="file" value=file_path_move />
                                    <input type="hidden" name="table" value=table_name_move />
                                    <input type="hidden" name="cell_id" value=cell.id.to_string() />
                                ...
                                <form method="post" action=format!("/projects/{}/table/notebook/cell-delete", project_id_del)>
                                    <input type="hidden" name="file" value=file_path_del />
                                    <input type="hidden" name="table" value=table_name_del />
                                    <input type="hidden" name="cell_id" value=cell.id.to_string() />
    ...
    view! {
        <div>
            ...
            <div style="display:flex; gap:0.5rem;">
                ...
                <form method="post" action=format!("/projects/{}/table/notebook/cell-create", project_id_new)>
                    <input type="hidden" name="file" value=file_path_new />
                    <input type="hidden" name="table" value=table_name_new />
