...
use super::handlers::load_project_kanban_columns;
use super::handlers::redirect_error;
use super::handlers::redirect_notice;
use super::handlers::resolve_project;
use crate::app::pages::template_library_page::CompiledPreview;
use crate::app::pages::template_library_page::ShareTargetOption;
use crate::app::pages::template_library_page::TemplateDetailPage;
use crate::app::pages::template_library_page::TemplateGalleryPage;
use crate::app::pages::template_library_page::TemplateShareDisplay;
...
pub fn build_template_router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/templates/:id/versions/:version_id/preview.pdf",
            get(latex_template_preview_pdf_action),
        )
        .route(
            "/templates/:id/visibility",
            post(update_template_visibility_action),
        )
        .route("/templates/:id/share", post(add_template_share_action))
        .route(
            "/templates/:id/share/:share_id/delete",
            post(remove_template_share_action),
        )
        .route("/templates/:id/delete", post(delete_template_action))
        .route(
            "/templates/create-from-kanban",
            post(create_template_from_kanban_action),
        )
        .route(
            "/templates/:id/publish-version-kanban",
            post(publish_kanban_version_action),
        )
        .route(
            "/templates/create-from-note",
            post(create_template_from_note_action),
        )
        .route(
            "/templates/:id/publish-version-note",
            post(publish_note_version_action),
        )
        .route(
            "/projects/:id/knowledge/kanban/apply-template",
            post(apply_kanban_template_action),
        )
        .route(
            "/projects/:id/note/apply-template",
            post(apply_note_template_action),
        )
        .route(
            "/templates/create-from-file",
            post(create_template_from_file_action),
        )
        .route(
            "/templates/:id/publish-version-file",
            post(publish_file_version_action),
        )
        .route(
            "/projects/:id/apply-file-template",
            post(apply_file_template_action),
        )
        ...
        .route(
            "/templates/apply-to-kanban",
            post(apply_kanban_template_from_library_action),
        )
        .route(
            "/templates/apply-to-note",
            post(apply_note_template_from_library_action),
        )
        .route(
            "/templates/apply-to-file",
            post(apply_file_template_from_library_action),
        )
}
...
async fn template_gallery_page(
    ...
    let templates = repo
        .list_visible_templates(user.id, kind.as_deref())
        .await
        .unwrap_or_default();
...
async fn template_detail_page(
    ...
    if matches!(template.kind.as_str(), "typst" | "slides") {
        for version in &versions {
            if let Ok(files) = template_library::files_from_content(&version.content) {
                let preview =
                    compile_typst_files_for_preview(&files, template.kind == "slides").await;
    ...
    if is_owner {
        for share in repo.list_template_shares(id).await.unwrap_or_default() {
            let label = if let Some(org_id) = share.org_id {
                ...
                match repo.get_team_by_id(team_id).await.ok().flatten() {
                    | Some(team) => {
                        let org_name = repo
                            .get_organization_by_id(team.org_id)
                            .await
                            .ok()
                            .flatten()
                            .map(|o| o.name)
        ...
        for org in repo.list_organizations(500).await.unwrap_or_default() {
            share_targets.push(ShareTargetOption {
                value: format!("org:{}", org.id),
                label: format!("🏢 {} (org)", org.name),
            });
            for team in repo.list_teams_by_org(org.id).await.unwrap_or_default() {
                share_targets.push(ShareTargetOption {
                    value: format!("team:{}", team.id),
                    label: format!("　└ {} / {}", org.name, team.name),
                });
            }
        }
    }
...
async fn latex_template_preview_pdf_action(
    ...
    let Some(version) = repo
        .get_template_version_by_id(version_id)
        .await
        .unwrap_or(None)
    else {
    ...
    let Some(template) = repo
        .get_template_by_id(version.template_id)
        .await
        .unwrap_or(None)
    else {
...
async fn update_template_visibility_action(
    ...
    let visibility = if matches!(payload.visibility.as_str(), "shared" | "public") {
        payload.visibility
    } else {
        "private".to_string()
    };
    let _ = state
        .db
        .repository()
        .update_template_visibility(id, &visibility)
        .await;
...
async fn add_template_share_action(
    ...
    let (org_id, team_id) = match payload.share_target.split_once(':') {
        | Some(("org", rest)) => {
            match Uuid::parse_str(rest) {
                ...
                | Err(_) => {
                    return redirect_error(&format!("/templates/{id}"), "Invalid org selection")
                },
            }
        },
        | Some(("team", rest)) => {
            match Uuid::parse_str(rest) {
                ...
                | Err(_) => {
                    return redirect_error(&format!("/templates/{id}"), "Invalid team selection")
                },
            }
        },
        | _ => {
            return redirect_error(
                &format!("/templates/{id}"),
                "Select an org or team to share with",
            )
        },
    };
    if repo.add_template_share(id, org_id, team_id).await.is_err() {
        return redirect_error(
            &format!("/templates/{id}"),
            "Failed to add share (maybe it already exists)",
        );
    }
...
async fn delete_template_action(
    auth: Option<AuthUser>,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Response {
    ...
    let template = repo
        ...
        .map_err(|e| {
            WebError::BadRequest(format!(
                "Failed to create template (slug may already be in use): {e}"
            ))
        })?;
...
async fn create_template_from_kanban_action(
    auth: Option<AuthUser>,
    headers: HeaderMap,
    State(state): State<AppState>,
    ...
    if !apich_db::IdentityPermissionResolver::can_access_project(
        state.db.pool(),
        user.id,
        project.id,
    )
    ...
    match create_template_and_first_version(
        &state,
        user.id,
        "kanban",
        &payload.name,
    ...
    .await
    {
        | Ok(template_id) => {
            redirect_notice(
                &back,
                &format!(
                    "Published template '{}' (view it at /templates/{})",
...
async fn publish_kanban_version_action(
    auth: Option<AuthUser>,
    headers: HeaderMap,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
        ...
        .await
    {
        | Ok(_) => {
            redirect_notice(
                &back,
                &format!(
                    "Published version {} to your template",
                    payload.version_label
                ),
            )
        },
        | Err(e) => {
            redirect_error(
                &back,
                format!("Failed to publish version (label may already exist): {e}"),
            )
        },
    }
}
...
async fn create_template_from_note_action(
    auth: Option<AuthUser>,
    State(state): State<AppState>,
    ...
    let back = format!(
        "/projects/{}/note?file={}&view=editor",
        project.id,
...
async fn publish_note_version_action(
    auth: Option<AuthUser>,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
...
async fn apply_kanban_template_action(
    auth: Option<AuthUser>,
    State(state): State<AppState>,
    ...
    let repo = state.db.repository();
    let Some(version) = repo
        .get_template_version_by_id(payload.template_version_id)
        .await
        .unwrap_or(None)
    else {
...
async fn apply_note_template_action(
    auth: Option<AuthUser>,
    State(state): State<AppState>,
    Path(id_or_slug): Path<String>,
    ...
    if state
        .project_manager
        .write_file(project.id, user.id, &file_name, &body)
        .await
        .is_err()
    {
...
async fn create_template_from_file_action(
    auth: Option<AuthUser>,
    State(state): State<AppState>,
    ...
    let content = match template_library::read_files_from_project(
        &project.storage_path,
        std::slice::from_ref(&payload.file),
    ) {
        | Ok(c) => c,
        | Err(e) => return redirect_error(&back, e),
    };
...
async fn publish_file_version_action(
    auth: Option<AuthUser>,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
...
async fn apply_file_template_action(
    auth: Option<AuthUser>,
    State(state): State<AppState>,
    Path(id_or_slug): Path<String>,
    ...
    match template_library::apply_files_content_to_project(
        &project.storage_path,
        &version.content,
        &payload.dest_subdir,
    ) {
        | Ok(written) => {
            if let Ok(vcs) = apich_vcs::api::ProjectVcs::open_or_init(&project.storage_path) {
                let _ = vcs
                    .snapshot_if_changed(format!("Applied template files: {}", written.join(", ")));
            }
            redirect_notice(
                &back,
                &format!("Wrote {} file(s) from the template", written.len()),
            )
        },
        | Err(e) => redirect_error(&back, e),
    }
...
async fn ensure_slide_helpers_if_applicable(
    ...
    if let Ok(Some(template)) = repo.get_template_by_id(template_id).await {
        if template.kind == "slides" {
            let _ = crate::services::cargo_slide_helpers::ensure_cargo_slide_helpers(storage_path)
                .await;
        }
    }
}
...
/// picker is.
async fn apply_kanban_template_from_library_action(
    auth: Option<AuthUser>,
    State(state): State<AppState>,
...
async fn apply_note_template_from_library_action(
    auth: Option<AuthUser>,
    State(state): State<AppState>,
...
async fn apply_file_template_from_library_action(
    auth: Option<AuthUser>,
    State(state): State<AppState>,
...
async fn compile_typst_files_for_preview(
    files: &[template_library::TemplateFile],
    is_slides: bool,
) -> CompiledPreview {
    ...
    if tokio::fs::create_dir_all(&dir).await.is_err() {
        return CompiledPreview {
            svg_pages: Vec::new(),
            error: Some("Failed to create a preview workspace".to_string()),
        };
    }
    ...
    let result = crate::services::document_renderer::DocumentRenderer::compile_typst(
        &dir, &main_file, false,
    )
    .await;
...
pub async fn list_own_templates_of_kind(
    state: &AppState,
    user_id: Uuid,
    kind: &str,
) -> Vec<apich_db::Template> {
    state
        .db
        .repository()
...
pub async fn list_visible_templates_of_kind(
    state: &AppState,
    user_id: Uuid,
    kind: &str,
) -> Vec<apich_db::TemplateWithLatestVersion> {
    state
        .db
        .repository()
        .list_visible_templates(user_id, Some(kind))
        .await
        .unwrap_or_default()
}
