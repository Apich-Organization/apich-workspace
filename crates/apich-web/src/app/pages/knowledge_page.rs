...
fn render_kanban_template_panel(
    ...
    let apply_panel = (!apply_options.is_empty()).then(|| {
        view! {
            <form method="post" action=format!("/projects/{}/knowledge/kanban/apply-template", project_id) class="inline-form" style="display:flex; gap:0.5rem; margin-top:0.6rem; align-items:center;">
    ...
    view! {
        <details style="margin-bottom:1.25rem; background:var(--bg-muted); border-radius:10px; padding:0.85rem 1rem;">
            ...
            <div style="margin-top:0.6rem;">
                <form method="post" action="/templates/create-from-kanban" class="inline-form" style="display:flex; gap:0.5rem; flex-wrap:wrap; align-items:center;">
                    <input type="hidden" name="project_id" value=project_id.to_string() />
