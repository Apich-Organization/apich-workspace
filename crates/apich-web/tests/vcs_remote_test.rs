...
async fn test_vcs_remote_bundle_clone_and_pat_authenticated_push() {
    ...
    let anon_client = reqwest::Client::builder().redirect(reqwest::redirect::Policy::none()).build().unwrap();
    let anon_get = anon_client.get(format!("{}/vcs-remote/{}/bundle", base_url, proj_id)).send().await.unwrap();
    assert_eq!(anon_get.status(), StatusCode::UNAUTHORIZED);
    let anon_post = anon_client.post(format!("{}/vcs-remote/{}/bundle", base_url, proj_id)).body(vec![0u8; 4]).send().await.unwrap();
    assert_eq!(anon_post.status(), StatusCode::UNAUTHORIZED);

    // --- "Clone": download the bundle using ONLY the PAT (no session cookie at all) ---
    let pat_client = reqwest::Client::builder().redirect(reqwest::redirect::Policy::none()).build().unwrap();
    ...
    assert!(bundle_bytes.starts_with(&[0x1f, 0x8b]), "response should be a real gzip stream");

    let client_temp = test_temp_dir();
    let client_root = client_temp.path().join("cloned_via_pat");
    let client_vcs = ProjectVcs::import_bundle(&bundle_bytes[..], &client_root).expect("import cloned bundle");
    ...
    std::fs::write(client_root.join("pushed_via_pat.txt"), "pushed from a PAT-authenticated client\n").unwrap();
    let pushed_snap = client_vcs.snapshot("Pushed via PAT-authenticated apich remote push").unwrap();
    assert_ne!(pushed_snap.id, original_head);

    let mut push_bytes = Vec::new();
    client_vcs.export_bundle(&mut push_bytes, apich_vcs::BundleOptions::default()).unwrap();
    ...
    assert!(accepted.iter().any(|b| b == "main"), "expected main branch to be accepted, got: {push_json}");
    assert!(push_json["rejected_branches"].as_array().unwrap().is_empty());
