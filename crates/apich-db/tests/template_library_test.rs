...
use apich_db::{
    CreateOrganizationDto, CreateTemplateDto, CreateUserDto, Database, IdentityPermissionResolver,
    PostgresConfig, PostgresContainer, PublishTemplateVersionDto, UserRole,
};
...
async fn test_template_library_visibility_and_versions() {
    ...
    let db = Database::connect_admin(&config, "localhost").await.expect("Failed to connect admin");
    db.run_migrations().await.expect("Failed to run migrations");

    let pool = db.pool();
    let repo = db.repository();

    let make_user = |username: &str| CreateUserDto {
    ...
    let owner = repo.create_user(make_user("template_owner")).await.expect("create owner");
    let outsider = repo.create_user(make_user("outsider")).await.expect("create outsider");
    let org_member = repo.create_user(make_user("org_member")).await.expect("create org member");
    ...
    assert!(IdentityPermissionResolver::can_access_template(pool, owner.id, template.id).await.unwrap());
    assert!(!IdentityPermissionResolver::can_access_template(pool, outsider.id, template.id).await.unwrap());
    assert!(IdentityPermissionResolver::can_manage_template(pool, owner.id, template.id).await.unwrap());
    assert!(!IdentityPermissionResolver::can_manage_template(pool, outsider.id, template.id).await.unwrap());
    ...
    let latest = repo.get_latest_template_version(template.id).await.unwrap().expect("latest version");
    ...
    assert_eq!(versions[0].id, version2.id, "newest version must sort first");
    ...
    let dup = repo
        ...
        .await;
    assert!(dup.is_err(), "publishing the same version_label twice must fail");
    ...
    let outsider_visible = repo.list_visible_templates(outsider.id, None).await.unwrap();
    ...
    repo.update_template_visibility(template.id, "shared").await.unwrap();
    assert!(!IdentityPermissionResolver::can_access_template(pool, org_member.id, template.id).await.unwrap(), "not shared yet");

    repo.add_template_share(template.id, Some(shared_org.id), None).await.expect("add share");
    assert!(IdentityPermissionResolver::can_access_template(pool, org_member.id, template.id).await.unwrap());
    assert!(!IdentityPermissionResolver::can_access_template(pool, outsider.id, template.id).await.unwrap(), "outsider is in no shared org");
    // The owner keeps access regardless of sharing (owner-check short-circuits visibility).
    assert!(IdentityPermissionResolver::can_access_template(pool, owner.id, template.id).await.unwrap());

    let org_member_visible = repo.list_visible_templates(org_member.id, None).await.unwrap();
    ...
    repo.update_template_visibility(template.id, "public").await.unwrap();
    assert!(IdentityPermissionResolver::can_access_template(pool, outsider.id, template.id).await.unwrap());
    let outsider_visible_now = repo.list_visible_templates(outsider.id, Some("kanban")).await.unwrap();
    assert!(outsider_visible_now.iter().any(|t| t.id == template.id));
    let outsider_visible_wrong_kind = repo.list_visible_templates(outsider.id, Some("note")).await.unwrap();
    assert!(!outsider_visible_wrong_kind.iter().any(|t| t.id == template.id), "kind filter must exclude non-matching templates");
    ...
    assert!(repo.get_template_by_id(template.id).await.unwrap().is_none());
    assert!(repo.list_template_versions(template.id).await.unwrap().is_empty());
}
